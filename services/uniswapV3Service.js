...
class UniswapV3Service {
  ...
  async queryPool(poolAddress, network) {
    ...
    if (!networkConfig || !networkConfig.dex.uniswapV3) {
      throw new Error(`Uniswap V3 not supported on ${network}`);
    }
    ...
    try {
      ...
      return {
        ...
        contracts: {
          pool: poolAddress,
          factory: networkConfig.dex.uniswapV3.factory
        },
        explorer: `${networkConfig.explorer}/address/${poolAddress}`
      };

    } catch (error) {
      throw new Error(`Failed to query Uniswap V3 pool: ${error.message}`);
    }
  }

  async queryPoolAcrossNetworks(poolAddress) {
    for (const [key, network] of Object.entries(NETWORKS)) {
      if (network.dex.uniswapV3) {
        try {
...
module.exports = new UniswapV3Service();
